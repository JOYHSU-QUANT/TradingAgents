...
- **One engine run for both entry points** (refactor plan v2, T2-d — PR 7 of
  ...
  they run inside `drive` before the failure reaches `run_engine`. Their
  words are unchanged. The traceback is still the engine's own exception on
  main's logger, and its frames now start in `engine_drive`. Build and drive
  stay two calls so the one-shot's `Running TradingAgents engine ...` line
  still prints after the graph is built, and not when building it fails.
...
- **The live lane's composition root moves out of `cli/`: `live/wiring.py`
  ...
  the `RealOrderGate` plus `HyperliquidSignedClient` pair (the config-only
  `live` gate check has one too, when a key is present), two of the recovery
  components
  ...
  (`no_live_block`, `paper_mode`, `mode_not_accepted`) — it prints nothing,
  because each command words the same refusal differently and each keeps
  its own wording table;
  ...
  checked before (after the paper refusal). The old copies' parse rung and
  three `risk:` rungs — `live:` parses; `risk:` present, parses, passes the
  §24 cross-check — are not on the ladder: `config.load_config` runs
  exactly those on every load that carries a `live:` block, and both
  commands load through it, so those rungs and their six CLI refusal lines
  (three per command) were unreachable before this change and are dropped
  with it; the ladder takes `load_config`'s output and says so. `cli/live.py`
  and `cli/smoke.py` call the
...
- **The direction probe pooled over several runs, with a block-bootstrap
  ...
  above 0, read only with blocks of six and when the runs make at least five
  blocks between them (with one block every draw is the same sample). Each run
  is scored on its own terms (its own flat band and train base rate, through
  the headline of `score --replay-db`, now also available one question at a
  time as `probe_score.headline_scores`), the pooled skill weighs every
  question alike, and the interval is a circular block bootstrap within each
  run: a run of n questions is redrawn as ceil(n/6) blocks of six consecutive
  questions from random starts, wrapping from its last question to its first,
  cut back to n (neighbouring 4h questions share a regime; resampling them one
  by one would draw too narrow an interval, and the circle leaves no short
...
- **A direction probe for the past papers: `python -m contrib.replay replay
  ...
  probability of being right, and only on the questions where the model asked
  for a target (19 of run 3-6's 127, measured 2026-09-24). With `--probe`, the
  replay puts the same questions to a *probe* instead of asking for a
  decision: one more completion per question and repeat, from the variant's
  model, temperature and cap, asking for up / down / flat probabilities 4h and
  24h ahead. It is a separate call and never part of the decision prompt
  (decided 2026-09-24): folded in, it would change the trader being measured.
  The system message is the probe's own text; the human message is the
  payload's `context_text` (and the variant's `extra_context`) under the
  engine's heading with the probe's instructions last, and no format block. A
  probe is a YAML file (`name`, `system`, `instructions`;
  `contrib/replay/probes/direction-v1.yaml` ships), keyed by a digest of its
  two texts, one name per digest, as variants are. An answer is taken when
  both horizons hold exactly `up` / `down` / `flat`, each a JSON number in [0,
  1], summing to within 0.02 of 1 (then normalised); anything else is stored
  as `invalid_probe`, counted and never asked again, so the scored sample does
  not lean towards the questions that are easy to answer. The split pin, the
  holdout's two flags and ledger row, the payload checks, resumption,
  `--limit`, `--dry-run` and the failure classes are the decision replay's;
  the probe runs on 4h runs only, where its two horizons are the scorecard's.
  `score --replay-db --variant` prints one more section per probe, marked
  against the scorecard's own later marks and flat band. Its headline scores
  one forecast per question, the mean of that question's valid repeats (a
  question with no valid forecast but an `invalid_probe` among its repeats
  stands in as the base rate, and the skill without those stand-ins is printed
  beside it): the multi-class Brier score and log loss against the base rate
  (the train segment's up / down / flat shares, the same answer to every
  question), the Brier skill score (0 or below: no direction beyond the base
  rate), the binary Brier score of up against down on the questions that moved
  (which does not depend on where the flat band sits), a temperature fitted on
  the train headline and scored on validation (and the holdout when it is
  read), and a reliability table of the most likely class with its expected
  calibration error; then each repeat on its own and the skill's median and
  range across repeats, as a stability check. A variant asked only the probe
  gets that section alone. The acceptance bar ("clearly above 0 on
  validation") is written into the plan before any run; pooling runs and a
  bootstrap interval come next. `replay.sqlite` moves to schema v3 (`probes`,
  `probe_answers`): a v2 store gains the two tables when a command that may
  create a store opens it, and any other command reads it as written. Two
  ideas from the same comparison (buberlo/jev-trader) are not taken, and the
  plan says why: labelling each decision's outcome while the run trades (the
  scorecard already labels them offline, and live labels would write the paper
  store), and a rule that decides in the model's place when the call fails (a
  4h cycle that fails only skips one decision, while an untested rule on a
  real position would change how the trader trades; revisit if the scorecard's
  unanswered count shows `api_failed` rounds clustering in large moves).
...
- **Past papers: put the paper trader's recorded questions to another model,
  ...
  `--segment holdout --holdout`, and the ledger row saying who asked it is
  written before its first payload is read. A
...
- **Each decision cycle now keeps the agents' reports beside the input
  ...
  both sidecars go through it. Every sidecar record is stamped
  `"schema": 1` (`SIDECAR_SCHEMA`) so a later reader tells the format from a
  successor's instead of guessing from which keys are present. `write_sidecar`
  takes the record as a builder callable and calls it inside its own
  never-raise, so a builder tripping over its input is a logged failure, not
  a lost decision. Two consequences for the existing `.usage.json`: it is now
  written atomically (`common/atomic_io.py` gained `atomic_write_bytes`; a
  restart mid-write can no longer leave a truncated file at the final path),
...
- **CME Bitcoin futures basis as a crypto market-analyst tool, off by default**
  ...
  **All or nothing.** The vendors that fetch two halves independently each
  ...
  may itself trail by make 19, and two more cover the closed days a holiday
  weekend puts between the earlier anchor and its newest traded hour.
  ...
  **Not a live reading, and how big is big.** CME closes for the weekend and a
  ...
  runs about a point high in the last days before it is withheld.
...
- **A daily macro-trend backdrop for the perp prompt, off by default**
  ...
  **Its own daily candle series**, fetched as a sixth market read (last in the
  chain, with a kill-switch refresh either side, so the longest UNREFRESHED
  run is unchanged) and only while the switch is on. Widening the existing
  `4h` window to reach 200 days instead would have moved RSI, both EMAs and
  the regime label — turning the feature on would then have silently changed
  ...
  decision quality is unverified and meant to be measured.

  Fail-closed as a whole, like the volume profile: fewer than 200 daily bars
  (every cycle, for a coin younger than that), a daily feed more than a day
  behind the context's as-of or ahead of it, or two exactly equal averages
  each drop the WHOLE section with a named WARNING rather than render a
  half-filled one. What is deliberately NOT checked is bar continuity — a
...
- **hyperliquid_perp: the acceptance report can now see a funding event stuck
  ...
  ``unknown_status``). A breadcrumb is erased only by an outcome that got
  strictly PAST the point where the recorded lane failed, so it always
  describes the last attempt that reached that far rather than an old one — a
  rate-less pass therefore disproves ``reader_failed`` and leaves every other
  verdict standing, because ``rate_at`` answers a venue outage with ``None``
  too and clearing more would erase a live corrupt-row verdict for the length
  of the outage. Posting clears everything, so a settled row never keeps the
  failure marker of the pass before it.
  ...
  Known trade-offs, so the warning is not read for more than it says. A failing
  lane now takes a brief write lock where the pass used to be read-only apart
  from the posting itself, including on the restart path. ``store_error: 0`` is
  not evidence that no store error happened: the breadcrumb is written to the
  same database that just refused a write, so that lane usually cannot record
  itself and degrades to a contained WARNING. And a flapping failure is
  under-reported, because the backfill runs every cycle boundary while
  ``validate`` reads one instant — the count says what was true at the last
  attempt, and the journal remains the only place to see a rate.
