...
- **sosovalue_macro: the oldest print of a series no longer drops the whole
  ...
  oldest print now parses as the empty string, the "no figure" meaning a
  pending ``actual`` already holds. The tolerance is scoped to exactly that
  row: ``actual`` and ``forecast`` are still required everywhere, and a
  newer row without ``previous`` still fails the event — a provider that
  renames or drops the field mid-series must surface as a disclosed gap,
  not as an empty Previous column served for a full TTL.
