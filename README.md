...
A completion-token cap works the same way: set `max_tokens` in your config (or `TRADINGAGENTS_MAX_TOKENS` in `.env`, positive integer). The interactive CLI applies a default of 8192 when neither is set and prints the cap and its source before the run. The library path (`DEFAULT_CONFIG`) leaves it unset so each provider keeps its own default — risky through gateway providers such as OpenRouter, where some upstreams treat a missing cap as "the model's full context" and deterministically reject every call (issue #177), so the graph emits one `RuntimeWarning` when a provider that routes to an upstream it cannot see (`openrouter`, `openai_compatible`) is about to run uncapped. The cap includes reasoning/thinking tokens: reasoning-first deep-think models spend part of it on thinking, so if answers come back truncated, raise it via `TRADINGAGENTS_MAX_TOKENS`.
