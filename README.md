...
A completion-token cap works the same way: set `max_tokens` in your config (or `TRADINGAGENTS_MAX_TOKENS` in `.env`, positive integer). The interactive CLI applies a default of 8192 when neither is set. The library path (`DEFAULT_CONFIG`) leaves it unset so each provider keeps its own default — risky through gateway providers such as OpenRouter, where some upstreams treat a missing cap as "the model's full context" and deterministically reject every call (issue #177), so the graph emits one `RuntimeWarning` when a gateway provider is about to run uncapped. The cap includes reasoning/thinking tokens.
