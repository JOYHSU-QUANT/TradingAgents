...
spreading positions, the net, its share of open interest, and the change
against the previous published report and the one four reports back (each
column labelled with its comparison date), read from the CFTC's public Socrata
API. It is the institution-side positioning read: who holds the regulated
future, where the Hyperliquid leaderboard says who holds the venue's perp and
the basis says what the future pays over spot — and the two are one picture:
the leveraged-fund short is usually the futures leg of the cash-and-carry
trade whose carry the basis pays, which both analysts' instructions now say.

**As-of is the publication date, not the report date.** Each report is as of
Tuesday's close (a Monday in a holiday week) and published Friday afternoon,
so a run on a Wednesday serves the previous week's report — the new one exists
as a fact about Tuesday but nobody could have read it yet, and filtering on the
report date would leak three days of the future into every mid-week backtest.
The module derives the publication date as the first Saturday after the report
date (the release is Friday evening in UTC and `curr_date` has no hour, so
Friday's cycles are withheld the report rather than served it early; anchored
to the weekday so a holiday-week Monday report does not derive to Friday) and
prints both dates. A release delayed past Saturday — a holiday Friday, the
batch releases after the late-2025 shutdown — is still served early in a
...
so. A Scale line, computed from the trailing 52 reports the analysis date could
see, says what size of weekly change is ordinary for each headline category
...
family's clock (stale serves up to 21 days, failed fetches never written); a
row the parser cannot read is dropped and its date logged, and when the
dropped row is newer than the report served the report says so. No report
...
To reduce variation you can lower the sampling temperature. Set `temperature` in your config (or `TRADINGAGENTS_TEMPERATURE` in `.env`); lower values make models that honor it more repeatable. The current curated models are reasoning-first and largely ignore temperature, so for tighter reproducibility use a non-reasoning model, which you can set explicitly via the Custom model ID option. On OpenAI and Azure gpt-5 models langchain-openai drops the temperature unless reasoning is off (`openai_reasoning_effort: none` / `TRADINGAGENTS_OPENAI_REASONING_EFFORT=none`, forwarded to both providers; the interactive picker offers no `none`, so set it in config or `.env`); the OpenAI and Azure clients emit one `RuntimeWarning` naming that remedy when a temperature you set was dropped.

A completion-token cap works the same way: set `max_tokens` in your config (or `TRADINGAGENTS_MAX_TOKENS` in `.env`, positive integer). The interactive CLI applies a default of 8192 when neither is set and prints the cap and its source before the run. The library path (`DEFAULT_CONFIG`) leaves it unset so each provider keeps its own default — risky through gateway providers such as OpenRouter, where some upstreams treat a missing cap as "the model's full context" and deterministically reject every call (issue #177), so the graph emits one `RuntimeWarning` when a provider that routes to an upstream it cannot see (`openrouter`, `openai_compatible`, or `openai` pointed at a proxy through `backend_url` / `OPENAI_API_BASE` / `OPENAI_BASE_URL`) is about to run uncapped. The cap includes reasoning/thinking tokens: reasoning-first deep-think models spend part of it on thinking, so if answers come back truncated, raise it via `TRADINGAGENTS_MAX_TOKENS`.
