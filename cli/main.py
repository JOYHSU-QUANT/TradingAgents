...
def run_analysis(checkpoint: bool | None = None):
    ...
    with Live(layout, refresh_per_second=4):
        ...
        # begin_checkpoint runs INSIDE the try: it opens the sqlite store before
        # it reads the existing step, so a raise there would otherwise leave the
        # store open with no end_checkpoint to close it.
        trace = []
        try:
            # Recompile with a checkpointer and inject the thread_id so
            # --checkpoint actually saves and resumes on the CLI path (#1249);
