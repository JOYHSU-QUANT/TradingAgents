...
來自 `tradingagents/graph/trading_graph.py`、`tradingagents/agents/schemas.py` 與
`tradingagents/agents/utils/agent_states.py` —— 穩定的公開行為：
...
contrib main.py（一次性）／ cli/_provider.py（daemon；同一條流程，多兩個 sidecar）
   ...
   │
   ├─ [daemon only] report_usage(...) 在 finally 寫 <payload>.usage.json  (integration/completion_usage.py)
