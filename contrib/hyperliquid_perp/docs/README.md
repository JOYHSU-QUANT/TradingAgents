...
| `exchanges/hyperliquid/funding_source.py` | ✅ | `HistoryFundingSource`：`ports.FundingSource` 的實作，從公開 fundingHistory 端點查過去整點的 funding rate（execution §6.5）；查不到回 `None`（事件留 pending）。 |
...
| `common/prompt_regime.py` | ✅ | `prompt_regime:` 行的唯一渲染函式（三個切段鍵、`n/a`、可選 `cycles=`）：`validate` 每桶一行、paper／live daemon 第一個 prompt 與每次翻桶各一行（`integration/decision_provider`）、`--context-only` 一行，三處同一文法可 grep（issue #163）。第一鍵的值 `PROMPT_VERSION` 也住這裡（issue #197；`cli` 保留 re-export），另外兩鍵從 prompt 本身算出、住 `domains/perp`。同理住這裡的還有 `position_section_omitted`：prompt 沒有 `Position:` 段時兩處（`integration/decision_provider`／`domains/perp/marginal_cost`）共用的 WARNING 模板，`reason=no_books`／`non_positive_equity` 是 grep 把手（RUNBOOK §7）。 |
...
| `runtime/position_facts.py` | ✅ | `read_books` → `BookFacts`：每次 `build_input`（cycle 的每一次 try）讀一次帳本／倉位／最新 fill 戳，同時餵 prompt 的 Position 段與 `ai_inputs` 列；`BookSource` 是 wiring 綁 store 的 callable 型別。 |
...
| `live/shutdown.py`＋`cli/paper.py::gate_restart` ✅ | 重構 v2 PR 8 | `live --run-id` 收尾的決策：`classify_shutdown(ShutdownFlags)` 決定 §18.2 sweep 留不留 SL/TP 與不乾淨的原因、`classify_exit(...)` 決定 exit code（兩者皆純函式）；`read_exit_state` 做收尾時的倉位與 safe mode 讀取、`sweep_on_exit` 跑 sweep 與 venue-identity 升級。stderr 措辭全留在 `cli/live.py`。paper 重啟時缺 OPENROUTER_API_KEY 或引擎建不起來，由 `gate_restart` 決定 protection-only 還是退出。 |
