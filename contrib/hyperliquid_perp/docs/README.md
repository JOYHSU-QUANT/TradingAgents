...
| `common/sidecar.py` | ✅ | payload 旁 sidecar 的唯一契約：`sidecar_path(payload, suffix)` 一種拼法（同目錄同 stem）、每份記錄蓋 `schema: SIDECAR_SCHEMA`（現在＝1，key 集合或值形狀變了就 bump）、`write_sidecar(build=…)` 在自己的 never-raise 裡呼叫 builder 再走 `atomic_io.atomic_write_bytes`（tmp→replace，半截檔不會出現在最終路徑）、值 JSON 帶不動就在葉子記 `str` 並留一行 WARNING、寫失敗只 log（`<what> sidecar could not be written; the decision is unaffected`）絕不影響 cycle。`.usage.json`（`integration/completion_usage`）與 `.reports.json`（`integration/decision_reports`）都走這裡。 |
