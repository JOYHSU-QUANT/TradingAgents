...
└── contrib/
    ...
    └── hyperliquid_perp/
        ├── exchanges/
        ├── domains/
        ├── common/                      # 跨層共用（enum guard · seam_guard.py 建構期 seam 守衛（callable 與物件兩型） · YAML coercion · decimal context · 常數含 CYCLE_INTERVAL · atomic write · instants.py 時戳解碼／whole-hours label／時距 label gap_label（macro_trend 與 research_signal 共用；freshness 的複合 `14h 12m 30s` 刻意不收斂）／epoch-ms 換算唯一實作／`*_seconds` 參數收斂 seconds_span · no_decision.py no-decision 升級政策）
