...
| `integration/decision_reports.py` | ✅ | `write_decision_reports`：每個 cycle 在 payload 旁寫 `<payload>.reports.json`——引擎得出 `final_trade_decision` 之前的四份分析師報告、兩場辯論、投資計畫與 trader 計畫（九個固定 key，缺的記 `null`），留給離線重放；規則同 usage sidecar（沒有列指向它、不在 hash 契約內、寫失敗只 log、模型看到的文字零改動）。 |
| `integration/completion_usage.py` | ✅ | `CompletionUsageCollector`（issue #182）：掛在兩個 LLM client 上的 callback handler，每筆 completion 記 node（langgraph `langgraph_node`）、model、output／reasoning tokens、stop reason 與是否截斷；provider 用它決定 `truncated_output`、印 `completion usage:` 與 `<payload>.usage.json`。量測失敗只 log、絕不影響決策。 |
...
| `common/sidecar.py` | ✅ | payload 旁 sidecar 的唯一契約：`sidecar_path(payload, suffix)` 一種拼法（同目錄同 stem）、`write_sidecar` 走 `atomic_io.atomic_write_bytes`（tmp→replace，半截檔不會出現在最終路徑）、值 JSON 帶不動就在葉子記 `str`、寫失敗只 log（`<what> sidecar could not be written; the decision is unaffected`）絕不影響 cycle。`.usage.json`（`integration/completion_usage`）與 `.reports.json`（`integration/decision_reports`）都走這裡。 |
