...
| `live/smoke.py` ＋ `live/validation.py` ＋ `signed_client.update_leverage` ＋ schema v8（`live_smoke_tests`）＋ CLI `live-smoke` 子命令與 `validate` live 分流 ✅ | PR 6 | §20.2 testnet smoke 18 項（§20.2 的 17 項＋第 18 項 emergency close——健康 cycle run 不產生 emergency close，故 §20.3 `emergency_close_test_passed` 只能來自刻意測試）逐項可跑／落 `live_smoke_tests`（append-only、latest-per-key 非 dry-run 為準）、`--dry-run` 純接線檢查不下單、`--gate-status` 純讀 gate、`--only` 選項；`smoke_gate_passed` 全過才允許 `live --loop` 進 testnet_live cycles（同 run；mainnet_tiny 依 §21.3 靠 testnet 已過的 smoke，故此 same-run gate 僅 testnet）；gate 三分流 missing／failed（交易所拒絕）／errored（harness bug）供真錢 go/no-go triage；restart 系列（15–17）走一次真 §19.1 recovery、其餘走 signed client 真動作（cleanup 失敗落 step detail 不吞）。§20.3／§21.4 live 驗收器（`validate` 依 run mode 分流）沿用 0/4/5：integrity failure（dedupe＝`fill_money_drift`、orphan_exchange_order、duplicate_fill_apply＝`exchange_fill_key` 重覆、exchange_position_mismatch、replay mismatch、unprotected 秒數＞0＝由 `protection_order_events` 重建：onset＝SL repair exhausted/blocked，close＝SL placed/modified／protection_cleared／emergency_close_triggered，per-symbol、per-window 負值 clamp、kill_switch_refresh_success_rate＜99%）→exit 5；cycles/orders 未滿、smoke 未跑或 failed/errored（可修後 `--only` 重跑、latest-per-key 覆蓋）→exit 4；全達→exit 0；§20.3 四個 `*_test_passed` 讀 smoke 15/16/17/18；**smoke gate 只 scope testnet_live**（§21.4 不含 smoke，mainnet run 的 `live_smoke_tests` 空表——否則 §21.4 永遠 exit 4）；mainnet 另 gate unresolved reconciliation／daily_loss，cycle 期間的 emergency close 走 warning（§21.4「非 bot bug」不可機器判定）。docs/RUNBOOK-live.md（前置→smoke→testnet_live 30 cycles→mainnet_tiny hard gate→監控/safe mode）（§20–§21）。 |
