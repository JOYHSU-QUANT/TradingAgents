...
| `domains/perp/context_builder.py` | ✅ | `market_data + account → PerpMarketContext`；計算 indicators 與 funding z-score，並在有設定視窗時掛上 volume profile。`position=` 是**必填** keyword（同 `exchange_time` 規則，不得靠忘記 kwarg 走到預設）：拿到 `PositionInputs` 就在這裡把 `Position:` 段按**同一個 snapshot 的 mark／funding** 定價掛上，`None` ＝ position-blind。`PerpMarketContext` 因此只有一條建構路徑（issue #134）。 |
...
| `common/config_coercion.py` | ✅ | 共用 YAML-coercion helpers（`config_overrides` · `decimal_from_yaml` · `int_from_yaml`），供 `RiskConfig` / `DecisionConfig` / `MarketDataConfig` / `PaperTradingConfig` / `LiveConfig` 使用（原 `domains/perp/`）。 |
| `domains/perp/market_data_config.py` | ✅ | typed `market_data:` 區塊（`MarketDataConfig`）：未知 key／錯型別／超出範圍的值在 config load 具名 exit 1，每個 key 的預設值只宣告在欄位上（沒有第二份清單，所以加 key 不會讓哪份文件過期），`volume_profile_window_candles ≤ candle_lookback` 跨欄位檢查在 `__post_init__`（`macro_trend_daily_lookback` **刻意沒有**這種跨欄位檢查：它數的是另一條序列的日線根數）；`autoresearch_signal` 有自己的轉換器，因為 YAML 把裸的 `off` 讀成布林，而那正是操作者會寫的字；`engine_bridge._build_context` 與 `build_market_context` 傳同一個解析好的物件（issue #96）。只 import `common/` 與 `schema`，layering 測試釘住。 |
| `domains/perp/margin.py` | ✅ | Hyperliquid maintenance-margin tier model（rate · continuity deduction · tier 選擇；phase2-execution §6.6.1）。§6 的基礎公式也在這裡各只寫一次：`position_notional`／`unrealized_pnl`／`account_equity`／`funding_cost`。`funding_cost` 是**成本符號**（正＝倉位付）的單一小時 funding；帳本的 `accounting.funding_pnl`（正＝收入）是它取負，prompt 的 8h 持有成本是它乘上 `HOLDING_COST_HOURS`——一個公式、一個取負（issue #134）。 |
...
| `persistence/backfill.py` | ✅ | `ai_inputs.format_fingerprint` 的離線回填（issue #163）：v11 前的 NULL 列從各自 payload JSON 的 `format_instructions` 重算指紋；只寫 NULL 格、只寫已有另外兩鍵的列（pre-v10 列不填，半組三鍵會變新桶）、檔案 bytes 要 hash 到列上的 `input_payload_hash`（`common/digest.py`，與 daemon 寫入端同一個函式）、缺檔／讀不到／改過的列保持 NULL 並計數。刻意不是 migration。入口 `export --backfill-format-fingerprint`；照 daemon 版面（`<db 目錄>/payloads/<run-id>/`，`common/store_layout.py` 唯一定義）一起搬到 db 旁的複本不帶 flag 也會自動找到（記錄路徑全數缺檔才試；issue #221）；沒照版面擺的才加 `--payload-root DIR`（每列只取記錄路徑的檔名到 DIR 下找，hash 規則不變；issue #197）。 |
| `common/sidecar.py` | ✅ | payload 旁 sidecar 的唯一契約：`sidecar_path(payload, suffix)` 一種拼法（同目錄同 stem）、每份記錄蓋 `schema: SIDECAR_SCHEMA`（現在＝1，key 集合或值形狀變了就 bump）、`write_sidecar(build=…)` 在自己的 never-raise 裡呼叫 builder 再走 `atomic_io.atomic_write_bytes`（tmp→replace，半截檔不會出現在最終路徑）、值 JSON 帶不動就在葉子記 `str` 並留 WARNING（`<what> sidecar: a <型別> value JSON cannot carry was stored as its str`，每型別每次寫一行）、builder 自帶 `schema` key 視為錯誤（log、不寫）、寫失敗只 log（`<what> sidecar could not be written; the decision is unaffected`）絕不影響 cycle。`.usage.json`（`integration/completion_usage`）與 `.reports.json`（`integration/decision_reports`）都走這裡。 |
| `common/prompt_regime.py` | ✅ | `prompt_regime:` 行的唯一渲染函式（三個切段鍵、`n/a`、可選 `cycles=`）：`validate` 每桶一行、paper／live daemon 第一個 prompt 與每次翻桶各一行（`cli._provider`）、`--context-only` 一行，三處同一文法可 grep（issue #163）。第一鍵的值 `PROMPT_VERSION` 也住這裡（issue #197；`cli._provider`／`cli` 保留 re-export），另外兩鍵從 prompt 本身算出、住 `domains/perp`。同理住這裡的還有 `position_section_omitted`：prompt 沒有 `Position:` 段時兩處（`cli._provider`／`domains/perp/marginal_cost`）共用的 WARNING 模板，`reason=no_books`／`non_positive_equity` 是 grep 把手（RUNBOOK §7）。 |
| `paper/accounting.py` | ✅ | 模擬 fill 的交易性 posting（`post_fill`／`apply_fill`，taker 0.045% 模型 fee）與 funding exactly-once（`record_funding`／`FundingResult`）（phase2-execution §6）。帳務公式、fill effect、genesis 與 replay 在 `runtime/accounting.py`（refactor plan v2 T1-d）。 |
...
| `runtime/asset_spec.py` | ✅ | `AssetSpec`（coin／szDecimals／margin schedule，qty step 與 price tick 建構時算好）、兩個精度 helper `qty_step_from_sz_decimals`／`price_tick_from_sz_decimals`，與 `build_asset_spec(market, coin)`（一次 meta 讀取＋建構；paper／live loop／live-smoke 三個站點都走這裡，refactor plan v2 T2-b）。 |
...
| `runtime/run_identity.py` | ✅ | `open_run(db_path, run_id, *, create)`：paper／live 兩個 daemon 開 store 的同一條前段——store 原樣開（不 migrate，issue #129）、讀 `runs` 列、`--create` 兩向互斥（缺 run 無 `--create`＝`missing_run`、有 run 帶 `--create`＝`run_exists`）；拒絕丟 `RunIdentityRefusal`（帶 `RunIdentityStage`，store 已關），不印字、措辭在 cli；回 `OpenedRun`（db／列／`is_restart`／`foreign_mode(lane)` 供各 lane 在原位置做 run-mode 檢查）。lease、SIGTERM、deferred migration 留在各 lane（順序不同）（refactor plan v2 T2-b）。 |
| `runtime/genesis.py` | ✅ | `write_genesis(db, *, run_id, mode, initial_balance_usdc, seeds, config_subset, created_at)`：兩個 lane 共用的 genesis 寫入——seed 從任何帶 `coin`／`size`／`entry_price` 的物件（`PositionSeed` Protocol：paper 的 config seed、live 的交易所倉位）折成 `PositionState`，config subset 以 `ensure_ascii=False, default=str` 序列化、蓋 `SCHEMA_VERSION`，再呼叫 `accounting.initialize_run`（refactor plan v2 T2-b）。 |
...
| `live/smoke.py` ＋ `live/validation.py` ＋ `signed_client.update_leverage` ＋ schema v8（`live_smoke_tests`）與 v9（`current_positions.exchange_liquidation_price`）＋ CLI `live-smoke` 子命令與 `validate` live 分流 ✅ | PR 6 | §20.2 testnet smoke 20 項（§20.2 的 17 項＋第 18 項 emergency close＋第 19／20 項 post-only 切片（2026-09-16 maker path）——健康 cycle run 不產生 emergency close，故 §20.3 `emergency_close_test_passed` 只能來自刻意測試）逐項可跑／落 `live_smoke_tests`（append-only、latest-per-key 非 dry-run 為準）、`--dry-run` 純接線檢查不下單、`--gate-status` 純讀 gate、`--only` 選項；`smoke_gate_passed` 全過才允許 `live --loop` 進 testnet_live cycles（同 run；mainnet_tiny 依 §21.3 靠 testnet 已過的 smoke，故此 same-run gate 僅 testnet）；gate 三分流 missing／failed（交易所拒絕）／errored（harness bug）供真錢 go/no-go triage；restart 系列（15–17）走一次真 §19.1 recovery、其餘走 signed client 真動作（cleanup 失敗落 step detail 不吞）。§20.3／§21.4 live 驗收器（`validate` 依 run mode 分流）沿用 0/4/5：integrity failure（dedupe＝`fill_money_drift`、orphan_exchange_order、duplicate_fill_apply＝`exchange_fill_key` 重覆、exchange_position_mismatch、replay mismatch、unprotected 秒數＞0＝由 `protection_order_events` 重建：onset＝SL repair exhausted/blocked，close＝SL placed/modified／protection_cleared／degraded_protection_cleared（＝這一段真的結束；§17.2 emergency close 只是「送出平倉單」，刻意不算 close——它的時間戳來自 engine 的 tick now，比 protection 自己蓋的 onset 更早，計進去會讓每次 §17.2 升級都量成 0 秒），per-symbol、per-window 負值 clamp、**窗數本身也 gate**（量到 0 秒但有窗不得讀成「從未無護倉」）、kill_switch_refresh_success_rate＜99%（**以時間計**＝(covered−outage)/covered，一次 outage 一列事件、長度算到下一列正面 wire 證據（`refreshed`／`armed` 證明排程再立、`disarmed` 證明觸發已清；`fired`、`disarm_failed`、撤單 sweep 都不結束它）；**超過當下實際生效 deadline 的沉默空窗也算 outage**（deadline 取自 armed／refreshed 列自己寫的 `deadline=...s`，genesis config 只當起始值）（沉默與 switch 燒過長得一樣，判成「停機不計」會讓 301s 的 outage 得 100%、299s 的反而失敗）；唯一豁免是空窗開頭為 `kill_switch_disarmed`（trigger 已清，刻意釋放），且該段兩邊都不計；視窗結束於最後一列事件而非 now，所以停掉的 run 再跑一次 validate 不會改變判決；樣本 < 100 時不判定，改記 shortfall——短 run 覆蓋時間太短，一次 30s 中斷就吃掉整段可用率）、**`kill_switch_cancel_triggered` 發生過**（dead man's switch 真的燒過：交易所已撤光錢包含 SL/TP，refresh 率稀釋不出來，兩個 profile 共用、不可補救）、**run 仍在 MANUAL safe mode**（§10.4 連虧 latch 等人工確認，期間 §13.1 仍讓 cycle 數繼續累積，可 `safe-mode --release` 後重驗））→exit 5；cycles/orders 未滿、smoke 未跑或 failed/errored（可修後 `--only` 重跑、latest-per-key 覆蓋）→exit 4；全達→exit 0；§20.3 四個 `*_test_passed` 讀 smoke 15/16/17/18；**smoke gate 只 scope testnet_live**（§21.4 不含 smoke，mainnet run 的 `live_smoke_tests` 空表——否則 §21.4 永遠 exit 4）；mainnet 另 gate unresolved reconciliation／**目前仍未解除的** daily_loss safe mode（曾破線但已隨 UTC 午夜釋放者走 warning，不再永久釘死整個真錢 run），cycle 期間的 emergency close 走 warning（§21.4「非 bot bug」不可機器判定）；`cycle_count` **只認 `completed`**（`invalid_output` 另計＋warning，不進 ≥30——§21.4 沒有 order count 背書，否則連續解不開輸出、一單沒下也會報 live_ready），unprotected 秒數只算「沒有一張足以**覆蓋**的 SL」的 blocked——判準與 §12.3 `_has_valid_sl` 一致（平倉方向＋`qty ≥ 倉位`）；covering 的那張單在 stamp 前會**向交易所 orderStatus 正面確認**（讀不到一律不 stamp、fail-closed），因為會走到這條分支就代表 kill switch 正下著，而過期的 deadline 早已讓交易所撤光整個錢包、只留本地 row 原封不動——「只讀本地那一張」不是更嚴，是換了一個此刻剛好失效的來源，covering 的那張單 order_id 記在事件上；最常見的 resize-blocked（後續切片成交、舊 SL 只蓋一部分）照樣開窗。**schema v9 ＋ 清算價鏡射**：`current_positions.exchange_liquidation_price` 由 reconciler 每 pass 由交易所鏡射（方向不符或 flat 寫 None），engine 直讀當 live SL 帶寬的上游——此前 engine 讀一個不存在的欄位而恆得 None，§3.6 `liquidation_too_close`／CLOSE_NOW 在 live 從未可達；欄位 `compare=False`（replay 身分不含它）、鏡射寫入為 advisory，且翻向／歸零時由 `upsert_current_position` 清除（反向殘值會讓健康部位被判 CLOSE_NOW）。docs/RUNBOOK-live.md（前置→smoke→testnet_live 30 cycles→mainnet_tiny hard gate→監控/safe mode）（§20–§21）。 |
