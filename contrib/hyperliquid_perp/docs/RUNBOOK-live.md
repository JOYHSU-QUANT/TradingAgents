...
| `<path> is a SQLite database, but not one of this project's stores`（exit 1；`paper`／`live`／`live-smoke`／`safe-mode`／`validate`／`export` 都會印同一句） | `--db` 指到了**別的應用程式的 sqlite 檔**——實務上就是路徑打錯。訊息會列出它在裡面找到的物件名（最多五個），拿那些名字就能認出是哪個檔。**檔案沒有被修改**（issue #174 之前不是這樣：純報表指令會先把 `schema_migrations` 寫進去才拒絕，而 `paper`／`live` 根本不拒絕，直接把本專案整套 schema 建進別人的資料庫）。修正 `--db` 即可；要在全新路徑開 store 就照 §3 用 `--create`（指到不存在的檔案是另一條訊息：`database ... does not exist`）。判定看的是本專案自己的表，不是 `schema_migrations`（Rails 的是 `version varchar`、golang-migrate 的是 `version uint64, dirty bool`，都用同一個表名），所以帶著**別人形狀**的 `schema_migrations` 的外來檔一樣會被拒絕；只有「整個檔案就一張 `schema_migrations`、是空的、欄位剛好是本專案的 `version`＋`applied_at`」才會被當成自己的空 store。若訊息額外說那張空的 `schema_migrations` 多半是**舊版本的本專案**留下的，那是修好之前造成的殘留。 |
| `<path> is a directory, not a database file`／`cannot open <path>: its directory ... does not exist`／`cannot read <path> to tell whether it is one of this project's stores`（都是 exit 1） | 同一組守衛的其他三種 `--db` 打錯法：指到目錄（少打檔名）、指到一個不存在（或根本不是目錄）的目錄底下、以及讀不到（權限等）。這三種以前都會落到 `main()` 的最後手段，印成 `fatal: unexpected error: OperationalError: unable to open database file` 的 exit 2；現在各自具名。照訊息修 `--db` 即可，store 不會被建到任何非預期的位置。 |
