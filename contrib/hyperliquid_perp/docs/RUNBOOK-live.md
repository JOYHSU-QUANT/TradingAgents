...
| `<path> is a SQLite database, but not one of this project's stores`（exit 1；`paper`／`live`／`live-smoke`／`safe-mode`／`validate`／`export` 都會印同一句） | `--db` 指到了**別的應用程式的 sqlite 檔**——實務上就是路徑打錯。訊息會列出它在裡面找到的物件名（最多五個），拿那些名字就能認出是哪個檔。**檔案沒有被修改**（issue #174 之前不是這樣：純報表指令會先把 `schema_migrations` 寫進去才拒絕，而 `paper`／`live` 根本不拒絕，直接把本專案整套 schema 建進別人的資料庫）。修正 `--db` 即可；要在全新路徑開 store 就照 §3 用 `--create`（指到不存在的檔案是另一條訊息：`database ... does not exist`）。判定看的是本專案自己的表，不是 `schema_migrations`（Rails、golang-migrate 也用那個表名），所以帶著 `schema_migrations` 的外來檔一樣會被拒絕。若訊息額外說某張空的 `schema_migrations` 是**舊版本的本專案**留下的，那是修好之前造成的殘留，可以安全刪掉。 |
