...
| `<path> could not be opened as a store: ...`（exit 1；`paper`／`live`／`live-smoke`／`safe-mode`／`validate`／`export` 都會印同一句） | 檔案在、**讀得到**、卻開不起來：開一個 store 本身就是寫入——`connect` 的 `PRAGMA journal_mode = WAL` 會把 journal mode 寫進資料庫檔頭。實務上是長度 0 又拒絕寫入的 `--db`（檔案本身的權限、所在目錄、或整個 volume 唯讀）。**訊息不預斷是哪一種**，列出的三項要逐一查。以前它不具名：`validate` 印成 exit 5 的 `store integrity failure`（那個代碼的意思是**帳本對不上**，會把人送去查根本不存在的帳務問題），擁有型指令則是 `fatal: unexpected error: OperationalError: ...` 這種沒指名 `--db` 的 exit 2；issue #235 之後改成具名 exit 1。這是 issue #210「讀不到」的另一半。**已經建好的 store 不會走這條**——它本來就是 WAL，那句 PRAGMA 對它是讀，`connect` 過得去（實測）；它真正在寫入時的失敗仍然照原樣炸，不會被包成「`--db` 打錯了」，因為只有**開檔**那一下被接起來。本專案的 schema 一列都沒有被建進去。 |
| `<path> is zero bytes, but <path>-wal beside it holds data`／`<path> is not there, but ...`（exit 1；同上六個指令） | 主檔**長度 0 或根本不存在**，旁邊卻有裝著資料的 `-wal`／`-journal`。這種資料庫的內容整個在那個 log 裡，不是空 store；成因是主檔被截斷、備份只還原了一半、或某個 writer 剛建完檔就死在第一次 commit 之前。以前會被判成「空的、是我們的」整套建進去，**而且開檔那一下就先把 log 刪掉**（實測 20KB 全沒了，本專案整套 schema 建在剩下的東西裡）——和 issue #174 同一種傷害，只是走的是那道守衛看不到的門（它問的是**主檔**裡有什麼）。現在具名拒絕，**兩個檔都沒有被碰過**（issue #236）。訊息**刻意不給還原步驟**：最直覺的那個（用任何 sqlite 工具打開這一對）正是會刪掉 log 的動作，先把兩個檔一起複製走，再查 `--db` 路徑。判準綁的是「主檔沒有 bytes **且** log 有 bytes」這個組合——`-wal` 本身不代表檔案是別人的（我們自己的 store 都有一個），而 0 長度的 `-wal`（SQLite 例行留下的）不算，那種空檔照舊整套建得起來。判定只問檔案系統、不問 SQLite：唯讀探針對這一對照樣會刪 log。 |
