...
4. **兩個「讀不到」的 key，自動了結的條件不一樣**（issue #66）：
   ...
   所以兩邊都是「交易所讀取一好一壞抖動一次、生一列」，都不會每輪一列，而且同一把 key 底下
   **只有最新那一列會是未了結的**（每一段都被結束它的那次讀取蓋掉），`--status` 與 §21.4
   看到的仍是一個活的故障。真正的差別是**還有沒有別的東西會關掉那一列**：`read_failed` 那張
   單留在 sweep 的游標裡，之後某一輪本來就會把它了結並順手蓋章，所以不必急；
   `local_terminal_read_failed` 最常見的那支答案（orderStatus 說終態）**不產生 case**，
   不在讀得到的當下蓋，就沒有任何後續會替它蓋。
