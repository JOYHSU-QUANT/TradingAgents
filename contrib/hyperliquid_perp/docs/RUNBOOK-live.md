...
- **長駐建議**同 paper（[RUNBOOK §3](./RUNBOOK.md)）：掛在會自動重啟的監管下，
  ...
  訊息**：只有「鎖著」會印 `database is locked`，權限／`-shm`／儲存問題 SQLite 都印
  同一句 `unable to open database file`（見 §8 那一列），要自己去查檔案權限與掛載。
