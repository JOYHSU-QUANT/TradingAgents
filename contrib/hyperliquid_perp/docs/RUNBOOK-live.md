...
| `<path> is zero bytes, but <path>-wal holds data`（六個指令都會印）／`<path> is not there, but ...`（**只有 `paper`／`live` 會印**，其餘四個先被 `database ... does not exist` 擋下）；都是 exit 1 | 主檔**長度 0 或根本不存在**，旁邊卻有裝著資料的 `-wal`／`-journal`。這種資料庫的內容整個在那個 log 裡，不是空 store；成因是主檔被截斷、備份只還原了一半、或某個 writer 剛建完檔就死在第一次 commit 之前。以前會被判成「空的、是我們的」整套建進去，**而且開檔那一下就先把 log 刪掉**（實測 20KB 全沒了，本專案整套 schema 建在剩下的東西裡）——和 issue #174 同一種傷害，只是走的是那道守衛看不到的門（它問的是**主檔**裡有什麼）。現在具名拒絕，**檔案都沒有被碰過**（issue #236）。**處置**：先把 log 複製走，再查 `--db` 路徑；訊息不叫你用 sqlite 工具去打開這一對，因為那正是會刪掉它的動作。**如果那個 log 你確定是垃圾**（例如你自己 `rm` 掉主檔、`-wal` 沒跟著刪），把它 **`mv` 移開而不是刪掉**，同一個路徑就會當成全新 store 建起來。判準綁的是「主檔沒有 bytes **且** log 有 bytes」這個組合——`-wal` 本身不代表檔案是別人的（我們自己的 store 都有一個，而且主檔從第一句 PRAGMA 起就有 4096 bytes，實測），而 0 長度的 `-wal`（SQLite 例行留下的）不算，那種空檔照舊整套建得起來。判定只問檔案系統、不問 SQLite：唯讀探針對這一對照樣會刪 log。訊息裡的 log 印的是**完整路徑**（`--db` 是 symlink 時它可能不在你打的那個目錄）；若某個 sidecar 連 `stat` 都問不出來，措辭會改成 `could not be measured and may hold data`，那是「量不到所以不敢動」而不是「量到了有資料」。 |
