...
- **長駐建議**同 paper（[RUNBOOK §3](./RUNBOOK.md)）：掛在會自動重啟的監管下，
  ...
  權限與掛載；長度 0 的檔走的是另一條車道、另一份清單，給的是可判讀的 errno；(c) 同一個 db 檔裡別的 run 還持著新鮮 lease、擋住這次升級的具名拒絕
  （訊息含 `is being driven by pid`），對方收工或 lease 過期後重跑就會好；
  (d) store **開不起來**的具名拒絕（訊息含 `could not be opened as a store`，
  issue #235），成因若是別的 process 握著它就重跑會好，若是權限／唯讀掛載就不會，
...
- **AI 從未回答，或回答無效而刻意未存**（`pending_raw_response` 是 NULL——兩者在列上同形，
  ...
  而不是 `invalid_output`；明文接受，見 phase2-spec §3.1 修訂框 (c)。要分辨是哪一種，看前一個
  process 的 log 有沒有 `did not parse to a decision`——列本身不猜）：沒有東西可武裝，改由 pump 每個 tick
...
的時間）：達到或超過 12h（＝`NO_DECISION_STREAK_THRESHOLD × CYCLE_INTERVAL`，與 no-decision 升級同一個
...
`venue_identity_fault`（見下）、`decision_adoption_wedged`（見下）。分兩型：
...
**意思**：§3.1 開機 adoption 失敗，而且**重試不會改變結果**（issue #205）。可自癒的有兩種、
都照舊重試：`sqlite3.OperationalError`（store 被鎖），以及**已經 arm 了失敗記錄之後**才在寫入
炸掉的那條（＝上面 §5「存壞的回覆」那條，pump 的 in-flight 分支只重試那一筆寫入）。剩下的
一律判定為「這列 row 本身壞了」——最典型的是 `find_in_progress_attempt` 對同一個 run 讀到
**兩列 `in_progress`** 而 `raise ValueError`（repository 刻意的 fail-loud，代表決策狀態機壞了），
其次是 `scheduled_at` parse 不出來。判定一次之後 driver 就**不再重試 adoption、也不會開新
cycle**（那個 stranded attempt 還握著 `next_decision_at`，硬開新 cycle 會每個 tick 撞主鍵），
並升 manual safe mode。
...
- journald 一行 ERROR：`decision driver startup adoption cannot complete and retrying will not
  ...
  traceback。之後每隔一個 cycle（4h）重印一行 ERROR `decision driver for <run> is WEDGED`
  ——**不是每個 tick**，這是它與上面那兩條可自癒分支（tick 頻率重複）最好認的差別。
- `safe-mode --status` → exit 4，reason `decision_adoption_wedged`，`detail` 寫著原因與
  `type(exc).__name__`。**例外**：若當下已經有另一個 reason 的 manual latch 站著，現態三欄會
  保留**先來的**那個 reason，這一條只會進 `safe_mode_events`（log 會明說）——此時解除那個舊
  reason 並不會解掉這個 wedge。
...
**撞到 wedge 的那個 process 不會退出**，這是刻意的：這種故障是決定性的，當場退出等於把重啟
交給監管而每次重啟都撞同一個例外、燒完 systemd 的 `StartLimitBurst` 之後服務永久停掉，屆時
真倉位只剩 SL/TP 掛著、**沒有任何 process** 在對帳、補保護單或 refresh kill switch。留著迴圈
至少這些都還在跑；manual latch 負責把「不會再有新決策」變成擋單、擋 verdict 的持久狀態
（§13.1／§13.6，SL/TP 與 §17.2 緊急平倉照 `PROTECTIVE_ORDER_ROLES` 豁免，仍送得出去）。

> **但 latch 站著的時候「重啟」是不會進迴圈的。** §19.1 的 verdict 含
> `not safe_mode_active`，所以 `live --run-id … --loop` 在 manual latch 下 `passed` 是
> False、**不會進 `_run_live_loop`**，直接印判定並 exit 4。所以下面的處置順序是
> **先修列 → 先解除 → 再重啟**，不要反過來。（跑著的 daemon 更簡單：解除之後它自己會在下一個
> tick 重跑 adoption，見下。）
...
python -m contrib.hyperliquid_perp safe-mode --run-id live-BTC --db live_trading.db \
  --release --reason "已人工 terminalize decision_attempts 的重複 in_progress 列"
```
...
`validate` 確認 `stranded_decision_cycle`／`in_progress_decision_attempts` 兩行都不見了。
...
| `store schema is vN but this build only knows vM`（拒絕開啟） | 這個 store 被**更新版**的 code migrate 過，現在用舊 binary 開它會用不認得的欄位寫穿它。跑回新版 code，或還原升級前的備份。**兩個 process 同時對落後 store 升級**時，輸家在寫鎖內逐版重讀、跑完再讀一次：贏家是同版就靜默跳過該版（不再 `duplicate column name` exit 2）；贏家是**更新版**就印這同一句——此時不是拒絕開啟，而是 migrate 階段的具名 exit 1（`paper`／`live-smoke` 會順手釋放剛取的 lease；`live` 此時尚未取 lease）。 |
...
| `<path> is a SQLite database, but not one of this project's stores`（exit 1；`paper`／`live`／`live-smoke`／`safe-mode`／`validate`／`export` 都會印同一句） | `--db` 指到了**別的應用程式的 sqlite 檔**——實務上就是路徑打錯。訊息會列出它在裡面找到的物件名（最多五個），拿那些名字就能認出是哪個檔。**檔案沒有被修改**（issue #174 之前不是這樣：純報表指令會先把 `schema_migrations` 寫進去才拒絕，而 `paper`／`live` 根本不拒絕，直接把本專案整套 schema 建進別人的資料庫）。修正 `--db` 即可；要在全新路徑開 store 就照 §2 用 `--create`（指到不存在的檔案是另一條訊息：`database ... does not exist`）。判定看的是本專案自己的表，不是 `schema_migrations`（Rails 的是 `version varchar`、golang-migrate 的是 `version uint64, dirty bool`，都用同一個表名），所以帶著**別人形狀**的 `schema_migrations` 的外來檔一樣會被拒絕；在**帶著 `schema_migrations` 的檔案**裡，只有「整個檔案就那一張表、是空的、欄位剛好是本專案的 `version`＋`applied_at`」才會被當成自己的空 store（完全沒有任何物件的檔案本來就算空的，另一回事）。若訊息額外說那張空的 `schema_migrations` 多半是**舊版本的本專案**留下的，那是修好之前造成的殘留。 |
| `<path> is a directory, not a database file`／`cannot open <path>: its directory ... does not exist`／`cannot open <path>: ... is not a directory`／`cannot read <path> to tell whether it is one of this project's stores`／`<path> could not be opened for reading: ...`／`<path> is not a regular file`（都是 exit 1） | 同一組守衛的其他幾種 `--db` 打錯法：指到目錄（少打檔名）、指到一個不存在或根本不是目錄的目錄底下、指到一個打得開路徑卻讀不到內容的檔、指到 FIFO 或裝置節點。**指到目錄**以前會落到 `main()` 的最後手段，印成 `fatal: unexpected error: OperationalError: unable to open database file` 的 exit 2；**父層不存在或不是目錄**這兩種只有走 `--create` 時才是 exit 2（其他指令本來就先被 `database ... does not exist` 的具名 exit 1 擋下）。現在各自具名。「父層不是目錄」在 Windows 印第三句、在 Linux 走第四句（`[Errno 20] Not a directory`）；第四句另外也接「父層目錄不可穿越」。照訊息修 `--db` 即可，store 不會被建到任何非預期的位置。**第五句**是「檔案在、`stat` 也答得出來，但打不開來讀」：可能是沒有讀權限、有 writer 在 5 秒 busy 等待之後還鎖著它、WAL store 旁的 `-shm` 建不出來、或底層儲存壞了。**訊息不預斷是哪一種，你也不能只靠它分辨**——「權限」與「`-shm` 建不出來」SQLite 印的是同一句 `unable to open database file`（實測），「鎖著」印 `database is locked`，I/O 故障印 `disk I/O error`（未實測），所以訊息列出的四項要逐一查。**空檔那條是另一份清單**：它走的是普通 read，不等 SQLite 的鎖、也不需要 `-shm`，所以只叫你查權限、**有沒有別的 process 獨佔開著它**（Windows 的 sharing 是強制的，這種情況跟權限被拒長得一模一樣）、以及儲存；它給的是 `[Errno 13] Permission denied` 這種可判讀的 errno。它以前不具名（`stat` 成功，所以前四句的守衛沒東西可判），`validate` 會印成 exit 5 的 `store integrity failure`（那個代碼的意思是**帳本對不上**，會把人送去查根本不存在的帳務問題），擁有型指令則是 `fatal: unexpected error: OperationalError: ...` 這種沒有指名 `--db` 的 exit 2；issue #210 之後改成具名 exit 1，長度 0 的檔也一樣（空檔不進 sqlite 探針——探針會把它旁邊的 `-wal` 當成過期的順手刪掉——改用一次普通的讀來問同一個問題）。**第六句**是 FIFO／裝置節點：它 `stat` 得出來、大小 0、又不是目錄，會被前面每一條當成空 store，然後去讀它——在 POSIX 上開一個 FIFO 來讀會擋到有 writer 出現為止（`open(2)` 語意，未實測），沒有任何 timeout 管得到，daemon 會直接卡住而不是拒絕；Windows 上這條守衛擋的是 `--db NUL`／`CON` 這種裝置節點。檔案都沒有被修改。 |
| `<path> could not be opened as a store: ...`（exit 1；`paper`／`live`／`live-smoke`／`safe-mode`／`validate`／`export` 都會印同一句） | 檔案在、**讀得到**、卻開不起來：開一個 store 本身就是寫入——`connect` 的 `PRAGMA journal_mode = WAL` 會把 journal mode 寫進資料庫檔頭。**訊息列出三項要逐一查、不預斷是哪一種**：(a) 檔案或它所在的目錄拒絕寫入、(b) 整個 volume 唯讀、(c) **有別的 process 握著它**。實測過的是長度 0 又拒絕寫入的 `--db`（`attempt to write a readonly database`），但接起來的是**開檔那一下所有的 `OperationalError`**，所以 `database is locked`（(c)，姊妹 daemon 還握著）與 `disk I/O error`（儲存壞了）也走這條——**後者要當硬體問題查，不要因為 exit 1 就當成打錯路徑**。以前它不具名：`validate` 印成 exit 5 的 `store integrity failure`（那個代碼的意思是**帳本對不上**，會把人送去查根本不存在的帳務問題），擁有型指令則是 `fatal: unexpected error: OperationalError: ...` 這種沒指名 `--db` 的 exit 2；issue #235 之後改成具名 exit 1。這是 issue #210「讀不到」的另一半。**已經建好的 store 不會走這條**——它本來就是 WAL，那句 PRAGMA 對它是讀，`connect` 過得去（實測）；它真正在寫入時的失敗仍然照原樣炸，不會被包成「`--db` 打錯了」，因為只有**開檔**那一下被接起來。本專案的 schema 一列都沒有被建進去。 |
| `<path> is zero bytes, but <path>-wal holds data`（六個指令都會印）／`<path> is not there, but ...`（**只有 `paper --create`／`live --create` 會印**——不帶 `--create` 的 `paper`／`live` 和其餘四個指令都先被 `database ... does not exist` 擋下）；都是 exit 1 | 主檔**長度 0 或根本不存在**，旁邊卻有裝著資料的 `-wal`／`-journal`。這種資料庫的內容整個在那個 log 裡，不是空 store；成因是主檔被截斷、備份只還原了一半、或某個 writer 剛建完檔就死在第一次 commit 之前。以前會被判成「空的、是我們的」整套建進去，**而且開檔那一下就先把 log 刪掉**（實測 20KB 全沒了，本專案整套 schema 建在剩下的東西裡）——和 issue #174 同一種傷害，只是走的是那道守衛看不到的門（它問的是**主檔**裡有什麼）。現在具名拒絕，**檔案都沒有被碰過**（issue #236）。**處置**：先把 log 複製走，再查 `--db` 路徑；訊息不叫你用 sqlite 工具去打開這一對，因為那正是會刪掉它的動作。**如果那個 log 你確定是垃圾**（例如你自己 `rm` 掉主檔、`-wal` 沒跟著刪），把它 **`mv` 移開而不是刪掉**，同一個路徑就會當成全新 store 建起來。判準綁的是「主檔沒有 bytes **且** log 有 bytes」這個組合——`-wal` 本身不代表檔案是別人的（我們自己的 store 都有一個，而且主檔從第一句 PRAGMA 起就有 4096 bytes，實測），而 0 長度的 `-wal`（SQLite 例行留下的）不算，那種空檔照舊整套建得起來。判定只問檔案系統、不問 SQLite：唯讀探針對這一對照樣會刪 log。訊息裡的 log 印的是**完整路徑**（`--db` 是 symlink 時它可能不在你打的那個目錄）；若某個 sidecar 連 `stat` 都問不出來，措辭會改成 `could not be measured and may hold data`，那是「量不到所以不敢動」而不是「量到了有資料」。 |
