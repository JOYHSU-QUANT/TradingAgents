...
| 每 4h 一筆 `api_failed`，error_message 是 `… freshness limit` | K 線 feed 停止推進。context 的 `as_of` 取自最後一根**收盤** K 線，健康 feed 不足一個 interval 舊；超過 `3 × candle_interval`（夾在下限與上限之間——下限＝`domains/perp/freshness.py` 的 `_MAX_CANDLE_AGE_FLOOR_MS`，目前 30 分鐘；上限＝三個決策 cycle，目前 12 小時；**但上限不會壓到一根健康 K 線以下**：`candle_interval: 1d` 的最新收盤 bar 在一天內會從 0 老化到 24h，所以 1d 的界限是「一根 bar ＋ 一個決策 cycle」＝28 小時——日線 feed 漏掉一根，最新收盤 bar 超過 28h 舊就拒跑（漏掉的 boundary 之後第二或第三個 cycle，看滾動排程落在哪）。目前出貨的 interval 沒有一個真的被 12 小時上限夾到（會被夾的是「大於一個 cycle、不超過三個」的 bar；4h 剛好是一個 cycle、走 `3 x 4h`，1d 走上面的加寬），所以 `… capped at` 這個由來標籤目前不會印出來。這些數字以程式碼的常數為準，訊息本身也會印出生效上限）就拒跑，不燒 LLM。訊息會印出該根 K 線的收盤時戳、實際年齡、生效上限與上限的由來（`3 x 4h`／`… raised to the 30m floor`／`one 1d bar plus one 4h decision cycle`）。**倉位不會被動到**：拒的只有 4h 一次的新決策，30 秒節奏的 monitor tick（清算／`gap_stop_fill`／SL・TP）照常跑——它讀的是 snapshot 的 mark price，與 K 線 endpoint 是不同資料路徑，K 線停了不代表保護瞎了。**訊息會直接指出成因**：年齡是拿**交易所自己的時鐘**量的（同一次抓取多讀一個 public `l2Book` 的 `time`），所以主機時鐘偏移不再與 feed 停滯混為一談——偏移大到足以獨力越過上限就寫「fix time sync (NTP)」、偏移在 1 分鐘與上限之間會說它「只解釋其中 X」要兩邊都查、主機時鐘與交易所一致則明寫是 feed 沒推進。照訊息指的方向查（交易所 K 線 API 狀態／`timedatectl`）。**不再是完全無聲的無限期狀態**：這類 cycle 的 `error_type` 記成 `stale_market_data`（不再與會自癒的環境失敗共用 `server_error`），連續第 3 筆起 log 從 WARNING 升成 ERROR，`validate` 也會把 `no_decision_streak ≥ 3` 列成 exit 4 的 `shortfall:`（該判準**不分 error class**——feed 停滯、l2Book 掛掉、連線問題一律計入，因為對操作員來說都是「連續 N 個 cycle 出不了決策」；報告另印 `stale_feed_refusal_streak` 供分辨。feed 恢復、下一個 cycle 出得了決策就自動歸零；run 停掉超過 2 個 cycle 後也不再套用，已封存的驗收 run 不會被永久判死）。`--context-only` 走同一套 guard：照樣渲染但印同一句警告並 exit 4。live run 的對應行為見 [RUNBOOK-live.md](./RUNBOOK-live.md)（tick 節奏不同，~10s）。 |
| error_message 是 `… AFTER the exchange's clock … a closed candle can lead it by` | 最新那根 K 線的收盤時戳落在**交易所時鐘**之後超過 1 分鐘（`_CANDLE_LEAD_TOLERANCE_MS`；與陳舊側的上限**無關**——一根真正收盤的 bar 不可能領先交易所時鐘，這 1 分鐘只是兩個交易所端時戳之間的量測誤差餘裕）。生產路徑的成因：本機時鐘走**快**，讓 `get_candles` 的 `close_time <= end` 篩選把**還沒收盤**的那根（OHLCV 不完整，ATR 被低估、RSI/EMA 偏斜）留了下來——主機快 H，落在 bar 最後 H 內的抓取拿到的就是這種 bar，其中離收盤還超過一分鐘的會被拒、最後一分鐘內的會放行（那根 bar 最多少一分鐘的成交；`1m` bar 因此永遠不會走到這個拒跑）。訊息印出本機比交易所快多少並要求修 NTP：用 `timedatectl` 修。**這種拒跑記成 `stale_market_data`、計入 no-decision streak**：排程是滾動式（完成時刻＋4h），抓取落在 bar 內的相位會慢慢漂移，主機一直快下去就會有一整段連續 cycle 被拒、湊滿 3 個就 exit 4——這是刻意的（時鐘不修就不該達標），`stale_feed_refusal_streak` 亮了而交易所 K 線 API 正常時，先查 `timedatectl`。訊息若**沒有**指出主機時鐘快了那麼多（說 `agrees with the exchange's`、或偏移不夠大），代表兩種情況之一：抓 K 線與讀交易所時鐘之間本機時鐘**往回跳**了（NTP step、休眠喚醒——查系統 log），或這份 context 不是來自 live 抓取（replay／手工 fixture）；訊息會把兩種都列出來。 |
