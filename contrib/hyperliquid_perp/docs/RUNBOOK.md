...
| 每 4h 一筆 `api_failed`，error_message 是 `every technical indicator failed` 或 `atr_14 is unavailable` | indicator 引擎（stockstats）壞掉或 `indicators:` 沒配 `atr_14`——regime 會被捏造成 RANGING，daemon 與 one-shot 同樣拒跑（不燒 LLM）。修 stockstats 相容性或把 `atr_14` 加回 `indicators:`。 |
