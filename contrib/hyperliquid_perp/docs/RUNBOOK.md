...
| `4` | 資料一致但 cycles 未滿 30；**或 `stale_feed_refusal_streak` ≥ 3**（最近連續 ≥3 個 cycle 都因 K 線 feed 停滯／主機時鐘偏移被拒，報告會印 `shortfall:` 行說明；feed 恢復後下一個決策 cycle 自動歸零） | 繼續掛著跑；stale streak 照 §7 `freshness limit` 列處置 |
