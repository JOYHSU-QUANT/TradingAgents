...
> （與 live 的 persist-retry 分流對齊）：**三筆 scheduler 自己的寫入**——回覆落地
> （`pending_raw_response`）、`ai_outputs` 稽核寫入，與終端 `api_failed` 記錄本身
> （issue #181：判決先武裝在記憶體、比照 live 的 `pending_fail`）——失敗時**不退出**，
> 決策（與已 gate 的 plan 登記，或已下的失敗判決）留在記憶體，下一次 poll 只重試那筆
> 寫入（絕不重問 AI、絕不重跑 gate、絕不重跑 §3.1 階梯；每次重試失敗都印 ERROR
> traceback——operator 的 export／validate 短暫持有
...
> 除它以外仍會退出的還有：上面那個 persist 逃生上限（三筆寫入共用同一個連續計數）、
> cycle 邊界那幾筆在所有守衛之外的排程寫入（`_execute` 呼叫 AI 前的 counter 預寫、
> 非最後一次 retryable 失敗後 `_record_failure` 的重試時戳回寫、新 cycle 的 attempt
> insert——這幾筆之前還沒有任何需要保住的持久事實，重啟會從列上的時戳重回階梯），
> 以及 cycle 收尾 best-effort 快照丟出的非 DB 例外。
> 武裝中的失敗判決不跨重啟：重啟時該列仍是 `in_progress`，依列上狀態重判（try 計數
> 已花完 → `interrupted`；還有剩 → 回 §3.1 階梯），與以前相同。
...
`cli._provider.PROMPT_VERSION` 是它的 re-export），讓 `ai_inputs.prompt_version` 在資料裡標出改版點。
...
- **在 stderr 不在 stdout**，前綴沿用這條 lane 既有的 `warning:`。理由是這個指令被記載的用法就是 grep
  stdout，把一句「上面那行有前提」放在會被 pipe 切走的那半邊，正好是它要防的失效。
- **不動 `prompt_regime:` 那一行**（另起一行）——三處共用同一個渲染函式、同一個字串要能 grep 是紀律。
- **它不保證有伴隨的 WARNING。** 少那一段時通常還有一句具名 WARNING 說是哪一種拒絕（檔案不在、過期、
  幣別不符、讀不出來），但**K 線視窗是空的時候，那份文件根本沒被讀過**——同樣是「沒有那一段」，卻一行
  WARNING 也沒有。所以那句話是條件句而不是承諾，而且會點名這個情況。
...
```sql
-- 每個 (規則, bias) 桶底下，模型要的方向怎麼分佈，以及 gate 放行了沒有。
-- ai_outputs.target_side 就是「模型要的那一邊」：RiskGate 從不改寫方向，
-- 連被拒的路徑也原樣帶著它（只夾 margin 百分比），所以不必另外找一個
-- 「request 版」的欄位。target_side 為 NULL 的是 maintain_current 與
-- 連方向都沒解析出來的 cycle。
-- risk_action 一定要在：沒有它，「偏多而且真的下單」跟「偏多但被 gate 擋下」
-- 會塌進同一個數字，而 run 6 問的是模型有沒有跟著走、不是有沒有成交。
...
**分母要自己決定**：這是 INNER JOIN，所以**根本沒走到 gate 的 cycle**（api_failed、重試用光）
不在結果裡；要把它們算進去就改 LEFT JOIN，那些列的 `o.*` 會是 NULL 自成一桶。
...
而且不會有任何列數警告。單一 run 範圍的查詢碰不到這件事，跨 run 的會。

**這條分辨規則有順序前提**：v13 要**先於開關被打開**部署。反過來做——先部署 #275 的 build 並把
`autoresearch_signal` 填上、之後才升 v13——那段期間的列會帶著 `autoresearch` shape 而兩欄是空的，
與 v13 之前的歷史**永久無法區分**。也補不回來：那兩個值當時只以散文形式存在 payload JSON 的
`context_text` 裡，要救得靠正規表達式去剖 prompt 文字，而不是 `persistence/backfill.py` 賴以成立的
純重算。

**部署帶 migration，換 schema 前先備份 DB，而且時機綁在換段。** 不是怕資料壞——這次是 `ALTER TABLE`
加兩個 nullable 欄，既有列全部維持有效。**備份保的是「退得回去」這個選項**：v13 的 build 開過的
store，只認得 v12 的 checkout 會**整個拒絕開啟**（`store schema is vN but this build only knows
vM`，連唯讀指令都拒），所以一旦升上去，要退回舊 binary 就只剩還原備份這條路。正因如此，**不要在
...
- **健康 resume 但引擎建不起來**（`EngineConfigError`；成因有二：引擎 import
  ...
  `TRADINGAGENTS_LLM_MAX_RETRIES=abc` 這種壞的 SDK 重試預算、或
  `TRADINGAGENTS_TEMPERATURE=abc` 這種壞的 sampling temperature——三個 env knob
...
| `1` | 操作錯誤（db／run 不存在，**或 `--db` 指到別的應用程式的資料庫、目錄、不是普通檔案的東西、一個打不開來讀的檔、一個開不起來的檔、或一個內容其實在 `-wal`／`-journal` 裡的檔**） | 檢查 `--db`／`--run-id`；訊息會說是哪一種，各列見 RUNBOOK-live §8。**「打不開來讀」與「開不起來」這兩種以前都印成 exit 5**（`store integrity failure`）——檔案權限問題被當成帳本問題，現在都歸這一列（issue #210／#235；後者是長度 0、讀得到但**寫不進去**的檔，因為開一個 store 本身就是寫入）。**「內容在 log 裡」那種以前更糟**：長度 0（或根本不存在）的主檔旁邊有裝著資料的 `-wal`，會被當成空 store 整套建進去，而且開檔那一下就先把那個 log 刪掉；現在具名拒絕、兩個檔都不碰（issue #236） |
...
| `context_shape` 少了結尾的 `position` token（prompt 沒有 `Position:` 段），log 有 `position section omitted` | 兩個成因渲染出**完全相同**的 prompt 與 `context_shape`，store 裡沒有欄位分得出（issue #161 拍板：接受現況、不加欄），**只能靠 journald 這行 WARNING 分辨**——兩處走同一個模板 `position section omitted (reason=<成因>): …`（`common/prompt_regime.position_section_omitted`，issue #197），grep `reason=` 這個把手而不是英文：`reason=no_books`（`cli._provider`：帳本還沒 seed，生產接線上到不了，出現代表接線或 store 有問題）vs `reason=non_positive_equity`（`domains/perp/marginal_cost`：權益 ≤ 0，該 cycle 省略倉位段是對的，gate 反正也會拒絕方向性目標）。成因詞彙表只有這兩個成員，兩處各有測試釘住。 |
