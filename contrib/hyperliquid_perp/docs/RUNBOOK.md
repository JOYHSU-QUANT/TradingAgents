...
| 每 4h 一筆 `api_failed`，error_message 是 `… freshness limit` | K 線 feed 停止推進。context 的 `as_of` 取自最後一根**收盤** K 線，健康 feed 不足一個 interval 舊；超過 `3 × candle_interval`（夾在 30 分鐘與 12 小時之間——上限＝三個決策 cycle）就拒跑，不燒 LLM。訊息會印出該根 K 線的收盤時戳、實際年齡、生效上限與上限的由來（`3 x 4h`／`… capped at`／`… raised to the 30m floor`）。**倉位不會被動到**：拒的只有 4h 一次的新決策，30 秒節奏的 monitor tick（清算／`gap_stop_fill`／SL・TP）照常跑——它讀的是 snapshot 的 mark price，與 K 線 endpoint 是不同資料路徑，K 線停了不代表保護瞎了。先查交易所 API 狀態，再用 `timedatectl` 確認主機時鐘——訊息本身就寫著這兩個成因「在這裡分不出來」（本機時鐘走**快**會產生一模一樣的量測結果）。**這是無限期狀態**：feed 一直不動就一直每 4h 拒一次、沒有升級或告警（issue #50 追蹤），所以看到這句要主動處理。`--context-only` 走同一套 guard：照樣渲染但印同一句警告並 exit 4。live run 的對應行為見 [RUNBOOK-live.md](./RUNBOOK-live.md)（tick 節奏不同，~10s）。 |
| error_message 是 `… AFTER the current time` | 產生這兩個時戳的**兩次讀時鐘之間**，本機時鐘跳了（主機從休眠醒來、NTP step、容器時鐘重新同步），或這份 context 不是來自 live 抓取。**方向不固定**：daemon 是先讀時鐘再抓資料，所以觸發它的是往**前**跳；one-shot／`--context-only` 是抓完才讀時鐘，觸發的是往**回**跳——所以訊息只說「跳了」不指方向。**另一個抓不到的情況是「時鐘一直慢」**（不是跳，是持續落後）——K 線視窗是用同一個時鐘算的，時鐘慢了 K 線也跟著被截，年齡看起來正常（issue #51 追蹤）。反過來「時鐘一直快」則會落在上一列（交易所沒有未來的 K 線可截，年齡讀起來就是大的）。用 `timedatectl` 確認 NTP 同步狀態與最近有沒有 step。 |
