...
| 每 4h 一筆 `api_failed`，error_message 是 `… freshness limit` 或 `host's clock is behind` | K 線 feed 停止推進（或本機時鐘偏掉）。context 的 `as_of` 取自最後一根**收盤** K 線，健康 feed 不足一個 interval 舊；超過 `3 × candle_interval`（夾在 30 分鐘與 12 小時之間——12h＝三個 4h 決策 cycle）就拒跑，不燒 LLM。訊息會印出該根 K 線的收盤時戳、實際年齡、生效上限與上限的由來。**倉位不會被動到**：拒的只有新決策，tick lane（kill switch refresh、§17 SL/TP 同步、§10.3 日損上限、§12.2 對帳）照常跑，SL/TP 用的是 30 秒節奏的 mark price、與 K 線是不同資料路徑。先查交易所 API 狀態，再用 `timedatectl` 確認主機時鐘；`host's clock is behind` 那一句是明確指向時鐘的版本（K 線不可能來自未來）。`--context-only` 走同一套 guard：照樣渲染但印同一句警告並 exit 4。 |
