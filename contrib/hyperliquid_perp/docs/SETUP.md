...
| `'market_data.volume_profile_window_candles' must be an integer …` / `must be >= 0` / `must be 0 (off) or at least 12` / `… exceeds 'market_data.candle_lookback'` | volume profile 的視窗設錯：非整數（含 `true`／`30.5`／`"30"`）、負數、1–11 太短、或大於 `candle_lookback` 永遠填不滿。四種都在 config load 具名 exit 1——**故意的**，因為這些錯誤在執行期唯一的症狀是「prompt 裡那段不出現」，跟 `0`（刻意關閉）分不出來。合法值見上面欄位表。 |
