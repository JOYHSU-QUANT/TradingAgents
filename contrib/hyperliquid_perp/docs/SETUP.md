...
| 開了 `market_data.macro_trend_daily_lookback` 但 prompt 裡沒有 `Macro trend (...)` 那段 | config 合法，所以這是**執行期**的 fail-closed，每次組 context 留一行具名 WARNING 說明是哪一種：`needs 200 daily candles … but only N are available`（幣的日線歷史不足——新上市幣會**每個 cycle**都出現這行，這種幣就把開關關掉，不是要修的故障）、`the newest daily candle is dated … and closed …h before this context's as-of, past the 24h …`（日線 feed 少了至少一整根——印日期與落後幾小時，不印 epoch 數字）、`the newest daily candle (dated …) closes …h AFTER this context's as-of`（反方向：**短周期序列落後**，不是日線壞掉，別去查日線那條）、`the daily SMA(50) and SMA(200) are exactly equal at …`（兩條均線完全相等，沒有排列可講）、`the 1d candle read for the macro-trend section failed`（那一次 REST 讀掛了——**決策照常做**，只少這一段）。整段省略而不是印 `n/a`，所以這些 cycle 會自動落在「沒有這段」的 `context_shape`，paper review 數得出來。 |
