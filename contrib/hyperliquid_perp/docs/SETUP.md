...
| 開了 `market_data.autoresearch_signal` 但 prompt 裡沒有 `Research signal (...)` 那段 | config 合法，所以這是**執行期**的 fail-closed，每個 cycle 留一行具名 WARNING 說明是哪一種：`could not be read`（檔案不存在／不是檔案／權限；訊息裡印的是展開 `~` 之後**實際去找**的路徑）、`not UTF-8`、`not valid JSON`（半寫入的檔案——producer 是先寫暫存再 rename，看到這個代表那條規則在哪裡被破壞了）、`was refused`（版本不符、多欄少欄、詞彙不合、欄位是多行或太長）、`is for ETH and this run trades BTC`（拿錯 coin 的文件）、`past the 2 x 4h bound`（producer 沒排程或掛了）、`AFTER this context's own bar`（寫文件那台主機的時鐘不對）。整段省略而不是印 `n/a`，所以這些 cycle 會自動落在「沒有這段」的 `context_shape`，paper review 數得出來。 |
