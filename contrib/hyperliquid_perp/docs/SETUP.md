...
| `invalid config — invalid market_data: config — unknown config key(s) …` / `… config key 'candle_lookback': expected an integer …` / `… 'market_data.candle_interval': unsupported candle interval …` / `… must be >= 1` | `market_data:` 區塊打錯 key 名（如 `volume_profile_window_candels`）、整數 key 給了 `true`／小數／非數字、`candle_interval` 不在六個合法值內（含大小寫錯的 `4H`）、或 `candle_lookback`／`funding_zscore_window_days` 小於 1。全部在 config load 具名 exit 1；以前打錯 key 名會靜默退回預設值、壞值要到 cycle 內取資料時才炸（daemon 會把它記成 `api_failed` 一直重試）。 |
