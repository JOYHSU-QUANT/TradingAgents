...
| `model` | 使用的 LLM model |
| `format_fingerprint`（schema v11 augmentation 欄，CSV 排在 `context_shape` 之後） | 當次 format 段——`decision_format_instructions` 渲染給模型的輸出格式契約文字——的**內容指紋**（`domains/perp/target_decision.format_fingerprint`：UTF-8 SHA-256 前 16 個 hex，例：`97aa0feaa4496d6f`）。第三個切段鍵：`prompt_version` 標 code 改版、`context_shape` 標段落結構，這欄標 format 段的**內容**——合法 margin 格線與有效上限（含被 `risk.max_target_margin_pct` 壓過的結果）渲染進那段文字，改這些 YAML 就換值，不需部署、不 bump、shape 不動。是指紋不是 shape：文字任何改動都換值。**prompt v5（2026-09-01，marginal-cost PR-B）起 `min_confidence`／`resize_min_confidence`／deadband 不再渲染成數字**，所以只改門檻的 YAML **不會**換這欄——門檻改動照 RUNBOOK 開新 run-id 切段，不能靠這欄。`validate` 依三鍵印每組的 cycle 數（`prompt_regime:` 行）。v11 之前的列為空（＝寫入時沒有這欄，不是另一個制度；payload JSON 存有當次 `format_instructions` 全文，`export --backfill-format-fingerprint` 可離線重算補回——只補 NULL 格、只補已有 `context_shape` 的列、檔案要 hash 到 `input_payload_hash`，見 RUNBOOK §6） |
| `context_shape`（schema v10 augmentation 欄，CSV 排在 `input_payload_hash` 之後） | 當次 market context 的**段落結構**字串（`domains/perp/prompt_context.context_shape`，例：`price\|market\|funding\|indicators(rsi_14,ema_20)\|volume_profile`）。與 `prompt_version` 並列的切段鍵：`prompt_version` 標 code 改版，`context_shape` 標「多一段／少一段」（含 `indicators` 清單重新排序——段落數不變但 prompt 不同）——包括只改 YAML 就會發生的（`market_data.volume_profile_window_candles`、`market_data.macro_trend_daily_lookback`、`indicators`）。prompt v4 起 daemon 的列多一段 `position`（倉位段有渲染即出現；open／flat 是帳戶狀態，不進 shape，用 `current_position_side` 切）。可能多一段 `macro_trend`（日線 SMA(50)/SMA(200) 背景，排在 `indicators(...)` 之後、`volume_profile` 之前；日線不足 200 根／過期／反而比 as-of 新／兩條均線完全相等／那一次 1d 讀失敗／本 cycle 沒有 4h K 線——這些 cycle 全部自動落在沒有這段的桶，要分辨是哪一種只能對照 journald 的 WARNING（最後一種不留 log））。PR C1 起可能多一段 `autoresearch`（research radar 的交接文件被接受即出現，排在 `volume_profile` 之後、`position` 之前；文件過期或被拒絕的 cycle 自動落在沒有這段的桶，而 bias／帶怎麼變都不進 shape）。**四個 optional token 的相對順序固定**＝`macro_trend` → `volume_profile` → `autoresearch` → `position`。只取結構，不取標籤裡的數字與隨資料有無出現的行。v10 之前的列為空 |
| `autoresearch_bias`（schema v13 augmentation 欄，CSV 排在 `format_fingerprint` 之後） | 當次 research signal 段**印給模型的那一邊**（`long`／`short`／`flat`，`domains/perp/schema.ResearchBias`），與 prompt 裡 `Side the rule holds after its latest bar:` 同一個字。跟 `context_shape` 的 `autoresearch` token 不同層：token 只說「那一段在不在」，這欄說「在的那些 cycle 看到的是哪一邊」——run 6 要問的「模型有沒有跟著那個 bias 走」只有這一欄能用 SQL 回答（issue #276）。與整列其他 context 欄同源，取自**渲染那份 prompt 的同一個 `PerpMarketContext`**，不是事後再讀一次交接文件（那時 radar 可能已經覆寫）。**空值有兩個意思**：那個 cycle 的 prompt 沒有這一段（開關預設關，多數列如此），或該列寫在 v13 之前。分辨靠同一列的 `context_shape`——shape 有 `autoresearch` 而這欄空＝v13 之前的歷史；shape 沒有＝那個 cycle 真的沒有這一段。兩個注意事項：寫 SQL 時 `context_shape` 要包 `COALESCE(...,'')`（v10 之前的列這一欄本身是 NULL，`NOT LIKE` 對 NULL 求值仍是 NULL，那些列會兩個桶都掉出去）；而且這條規則預設 **v13 先於開關被打開部署**，順序反過來的那段期間寫下的列與 v13 之前的歷史永久無法區分（見 RUNBOOK §4） |
| `autoresearch_strategy_id`（schema v13 augmentation 欄，CSV 排在 `autoresearch_bias` 之後） | 那一段指的是**哪一條規則**（`<experiment_id>#<trial_id>`，與 prompt 裡 `Research signal (rule …` 同一個字）。不與 bias 冗餘：radar 可以在 run 進行中 promote 新規則，沒有這一欄時兩條規則的 cycle 會加總成同一個分不開的數字。空值語意與 `autoresearch_bias` 相同，且兩欄恆同時有值或同時為空 |
...
| `status` | `in_progress` / `completed` / `api_failed` / `invalid_output`。**`api_failed` 也涵蓋一個窄窗口**：回答無效（PR #204 起不寫 `pending_raw_response`）之後 gate 被擋或 persist 失敗、接著重啟——重啟後的列沒有可續回覆，live 記 `api_failed`（`error_type` 空、`error_message` 寫 `no resumable response`）；paper 若還有預算則回到階梯重問、終態由重問決定，重啟時若第三次已經花掉則記 `api_failed`、`error_type=interrupted`。重啟當下都不會記 `invalid_output`。明文接受，見 phase2-spec §3.1 修訂框 (c)（issue #206） |
| `error_type` | `timeout` / `rate_limit` / `connection` / `malformed_response` / `stale_market_data` / `server_error` 等；成功留空。**`api_failed` 也可能留空**：非 Retryable 例外（通常是程式缺陷，live 自 Phase 3、paper 自 issue #134 起都記成 `api_failed` 續跑）沒有 §6.2 class，細節在 `error_message` 的 `non-retryable:` 前綴之後——不是壞掉的列。live 重啟 adoption 找不到可續回覆的那一列（`error_message` 寫 `no resumable response`）也留空，且**沒有** `non-retryable:` 前綴——按前綴篩缺陷列時它不會被選進來（見 `status` 列與 phase2-spec §3.1 修訂框 (c)）。`stale_market_data`＝交易所回答了、context 結構也完好，但它的**年齡**不可用（K 線 feed 停止推進，或量測年齡的兩個時鐘無法比對）——同樣不會自癒，`validate` 的 no-decision streak **不分 class**（issue #50）——它數的是「最近連續幾個 cycle 出不了決策」，`stale_market_data` 只決定 shortfall 的措辭、並讓報告另印 `stale_feed_refusal_streak` 這個子集。記成 `server_error` 會讓它與暖機不足、指標引擎壞掉這類會自癒的環境失敗混為一談。`malformed_response`＝交易所有回答、但回答無法解讀（誤路由的讀取、wire schema 漂移）；與 `connection` 分開記是因為連線中斷會自癒、這個不會，混在一起會讓按 class 的判讀把系統性壞掉的 feed 記成一次連線抖動 |
