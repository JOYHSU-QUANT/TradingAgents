...
若 AI API 成功回應，但 schema、型別或 cross-field validation 無效，則不再次呼叫 AI（唯一例外：下方修訂框 (c) 的重啟窗口）；依 fail-closed 規則記為 `decision_status = invalid_output`、套用 `maintain_current`、不建立 order，並保存原始 response（**保存位置見下方修訂框**——在 log，不寫進 `pending_raw_response`）。此 cycle 視為已完成，下一次以實際 `decision_at + 4 hours` 排程。

> **原始 response 保存在哪裡（2026-09-04 修訂）**：無效回覆**不寫進 `pending_raw_response`**，只在跳過該次寫入時以 WARNING 印出全文（grep `did not parse to a decision`）。`pending_raw_response` 的用途是「崩潰後可續」，而無效回覆不是可以續的決策，且它保存下來的文字不保證重 parse 得到同一個判決——非 str 的回答是以 `repr` 保存，重啟後它就是一個 str，`extract_json_block` 可能從中萃取出這一輪已經拒絕掉的方向性目標並下單（PR #204 實測）。代價：(a) 該文字的唯一副本在 log，不在 store 與 export；(b) 若 cycle 恰好崩潰在「回答之後、gate 之前」這個窗口，重啟後 live 走 fail closed、paper 回到 §3.1 重試階梯（可能多花一次 AI 呼叫），而不是從存下來的文字續跑。兩者都不下單。(c) 承 (b)，**這個窗口裡重啟後看到的那一列落在 `api_failed`，不會記成 `invalid_output`（2026-09-08 明文接受，issue #206）**：live 重啟時 adoption 看到的列沒有可續回覆，記 `api_failed`（`error_type` 空、`error_message` 寫 `no resumable response`、**沒有** `non-retryable:` 前綴——store 分不出「從未回答」與「回答無效而未存」，訊息刻意不猜；要分辨，看前一個 process 的 log 有沒有 `did not parse to a decision`）；paper 若還有預算則回到階梯：每次重啟恰好多問一次，cycle 的終態由重問決定（`completed`、`invalid_output` 或 `api_failed` 都可能），重問若遇 retryable 失敗就在同一 process 內續走階梯，總數不超過三次；重啟時若發現第三次已經花掉（不論前兩次是 timeout 還是同樣踩到窗口），記 `api_failed`、`error_type=interrupted`，不再問。後果是統計分桶：同一種「AI 有回答但無效」的 cycle，沒踩到窗口計入 `invalid_output_count`（paper 驗收還計入 30 輪），踩到就計入 `api_failed_count`、並讓 no-decision streak 加一而非歸零。不另設狀態、前綴或標記：窗口要同時滿足「回答無效、gate 被擋或 persist 失敗、重啟」三件事、沒有交易面傷害，且 (b) 已接受重問；能讓重啟不重問的做法（跳過寫入時在 `in_progress` 列留 `error_type` 標記與 `invalid_reason`、重啟時重建 fail-closed 判決直接進 gate）是兩條車道 post-answer persist lane 的生產碼改動，留作已知取捨。
