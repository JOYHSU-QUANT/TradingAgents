"""Account math and accounting replay shared by the paper and live lanes.

Two layers:

- **Pure math** (no I/O): the account/PnL/margin formulas of execution §6;
  :func:`compute_fill_effect`, which turns one modelled fill into its position
  delta, realized PnL, fee and wallet delta; and :func:`compute_live_fill_effect`,
  the same position transition with the exchange's fee and ``closedPnl`` in
  place of the model (phase3-spec §15). All-Decimal, deterministic — the same
  inputs always produce the same output, which is what makes replay meaningful.
- **Genesis and replay** (over :class:`~..persistence.db.Database`):
  :func:`initialize_run` writes a run's row, opening ledger and seed positions
  in one transaction; :func:`replay` rebuilds positions and the ledger from the
  committed fills, posted funding and — for a live run — the recorded
  accounting adjustments (§15.1 fee corrections, folded through
  :func:`adjustment_ledger_delta`), and compares them to the materialized
  ``current_*`` tables, reporting any mismatch (spec §5 accounting-replay
  acceptance check).

Posting is each lane's own: the paper lane's simulated fills and funding
settlements go through :mod:`..paper.accounting`, the live lane's exchange
fills through :mod:`..live.fills`. Both post with the formulas here, and
replay re-derives with the same ones, so the books and their check cannot
drift apart.
