"""Tests for paper accounting: simulated fill posting, funding exactly-once, replay over them."""
...
from contrib.hyperliquid_perp.persistence.ids import funding_event_id, slice_id
...
def test_posting_is_immune_to_ambient_decimal_context():
    ...
    assert runtime_accounting.replay(db, run_id="r1").is_consistent
    db.close()
...
def test_replay_is_immune_to_ambient_decimal_context():
    ...
    db = _db()
    _run_series(db)
    baseline = runtime_accounting.replay(db, run_id="r1")
    assert baseline.is_consistent
    original = decimal.getcontext().prec
    try:
        decimal.getcontext().prec = 6
        perturbed = runtime_accounting.replay(db, run_id="r1")
    finally:
        decimal.getcontext().prec = original
...
def test_record_funding_requires_initialized_run():
    ...
    fe_id = funding_event_id("r1", "BTC", _TS)
...
def test_replay_is_consistent_and_deterministic():
    db = _db()
    _run_series(db)
    r1 = runtime_accounting.replay(db, run_id="r1")
    r2 = runtime_accounting.replay(db, run_id="r1")
...
def test_replay_detects_corrupted_materialized_state():
    ...
    result = runtime_accounting.replay(db, run_id="r1")
...
def test_replay_recomputes_funding_from_stored_basis():
    db = _db()
    _run_series(db)
    assert runtime_accounting.replay(db, run_id="r1").is_consistent  # sanity before corruption
