...
def test_first_tier_must_start_at_zero():
    with pytest.raises(ValueError, match="first margin tier must start at lower_bound 0"):
        MarginSchedule(coin="BTC", tiers=(MarginTier(Decimal(100), Decimal(50)),))
...
def test_coin_must_be_non_empty():
    with pytest.raises(ValueError, match="coin must be non-empty"):
        MarginSchedule(coin="", tiers=(MarginTier(Decimal(0), Decimal(50)),))
...
def test_map_schedule_fallback_missing_max_leverage_raises_with_coin_context():
    # No marginTableId (single-tier fallback path) and no maxLeverage: _dec raises
    # MalformedResponseError (not ValueError), and the fallback must still add the
    # "maxLeverage for '<coin>'" context prefix rather than leak the bare field error.
    meta = {"universe": [{"name": "BTC", "szDecimals": 5}]}
    with pytest.raises(MalformedResponseError, match="maxLeverage for 'BTC'"):
        map_margin_schedule([meta, []], "BTC")
...
def test_margin_math_is_immune_to_ambient_decimal_context():
    ...
    baseline = MarginSchedule(coin="BTC", tiers=tiers)
    ...
    try:
        ...
        perturbed = MarginSchedule(coin="BTC", tiers=tiers)  # deductions baked at construction
