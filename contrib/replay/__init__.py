...
It reads the decisions the paper trader already recorded (``ai_inputs`` and
``ai_outputs``) and marks each one against the price that followed — the
scorecard — so that "did the model call it, and did the rule let it through"
becomes a table rather than an impression. That sentence is the package's
