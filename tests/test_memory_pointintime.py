...
@pytest.mark.unit
def test_memory_as_of_judges_the_trade_date_as_a_date():
    # "2024-5-10" is a usable date to the rest of the pipeline; compared as a
    # string the filter returned it unnormalized (so resolution dates, always
    # zero-padded, misfiltered against it), and a non-zero-padded date later in
    # the year than today's month sorted after today and disabled the filter.
    from tradingagents.graph.trading_graph import TradingAgentsGraph

    g = object.__new__(TradingAgentsGraph)
    assert g._memory_as_of("2024-5-10") == "2024-05-10"
    assert g._memory_as_of("not-a-date") is None
