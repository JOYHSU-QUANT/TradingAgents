...
@pytest.mark.unit
def test_global_news_search_count_is_clamped(monkeypatch):
    # The untrusted limit must be clamped before sizing the yf.Search call (#33).
    captured = {}
    ...
    monkeypatch.setattr(ynews.yf, "Search", FakeSearch)
    out = ynews.get_global_news_yfinance("2025-05-09", look_back_days=7, limit=10**9)
    assert captured["news_count"] == ynews.MAX_SEARCH_NEWS_COUNT
    assert "No global news found" in out
