...
from tradingagents.dataflows import interface, yfinance_common
...
class TestLoadOhlcvNoPoison(unittest.TestCase):
    ...
    def test_empty_download_raises_and_does_not_cache(self):
        empty = pd.DataFrame()
        with (
            mock.patch.object(yfinance_common.yf.Ticker, "history", return_value=empty),
            self.assertRaises(NoMarketDataError),
        ):
            yfinance_common.load_ohlcv("FAKE", "2026-01-01")
        # Nothing should have been written to the cache.
        self.assertEqual(os.listdir(self._tmp), [])

        # A second call must re-attempt the fetch (no poisoned cache served).
        with mock.patch.object(yfinance_common.yf.Ticker, "history", return_value=empty) as dl2:
