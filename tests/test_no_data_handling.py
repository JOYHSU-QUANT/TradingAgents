...
class TestLoadOhlcvNoPoison(unittest.TestCase):
    ...
    def test_empty_download_raises_and_does_not_cache(self):
        empty = pd.DataFrame()
        with (
            mock.patch.object(stockstats_utils.yf.Ticker, "history", return_value=empty),
        ...
        with mock.patch.object(stockstats_utils.yf.Ticker, "history", return_value=empty) as dl2:
