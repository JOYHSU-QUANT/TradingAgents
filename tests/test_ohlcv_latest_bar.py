...
Ported from upstream's ``stockstats_utils`` onto ``yfinance_common``. Here the
closeless row is dropped by ``_clean_dataframe``'s integrity guard (#38: a bar
missing any of OHLC is dropped, never filled), so the frame ends at the last
settled bar for the same reason and upstream's separate fill step has no
counterpart.
"""
...
def _run_load(monkeypatch, tmp_path, frame, curr_date):
    ...
    class _NoFetch:
        ...
        def history(self, *a, **k):
            raise AssertionError("should use the seeded cache, not fetch")

    monkeypatch.setattr(su.yf, "Ticker", _NoFetch)
...
def test_no_settled_bar_at_all_is_still_no_data(monkeypatch, tmp_path):
    ...
    with pytest.raises(NoMarketDataError, match="no usable OHLCV rows"):
