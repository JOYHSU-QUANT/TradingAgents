...
@pytest.mark.unit
def test_reddit_empty_window_is_named_not_reported_as_silence(monkeypatch):
    # Everything the live search returned fell outside the window: the answer
    # names the window and what the search could reach, rather than asserting
    # "no posts in the past 7 days" about a 2026-05 window.
    posts = [{"title": "NOW", "created_utc": _epoch("2026-08-30"), "source": "rss"}]
    monkeypatch.setattr(reddit, "_fetch_subreddit", lambda *a, **k: posts)
    out = reddit.fetch_reddit_posts(
        "AAPL", subreddits=("stocks",), inter_request_delay=0,
    ...
    assert "within 2026-05-01..2026-05-08" in out
    assert "in the past 7 days>" not in out
