    ...
    def market_analyst_node(state):
        ...
        if asset_type == "crypto" and not is_category_disabled(
            ...
            crypto_tools_message = (
                ...
                "risk reversal (RR25). Read DVOL's percentile as where the latest implied-vol "
                "reading sits versus the sample the report names on that line — it is a trailing "
                "year at most, and fewer readings when the feed is short or stalled, so cite the "
                "count rather than implying a full year. Read RR25 as which wing carries the "
                "higher implied "
                "vol — negative means the put wing does. RR25 is not comparable across tenors, "
                "so quote it with the expiry tenor the report prints: that is usually ~30 days "
                "but falls back to the next-nearest expiry when the nearest cannot be used. "
                "Report what the figures say rather than "
                ...
                "entirely for a past analysis date, and also for an asset Deribit does not "
                "list, so a report may carry no ATM/RR25 section at all — in which case do "
                "not substitute today's or BTC's skew for it; if the report is headed as a "
                "market-wide proxy the figures are "
