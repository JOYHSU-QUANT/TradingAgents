...
def get_macro_data(
    ...
    # Judged on the normalized date: ``date_refusal`` accepts "2026-6-5", and a
    # lexical min() sorts that after today - pinning a June backtest to
    # today's revisions. The observation bound is sent normalized for the
    # same reason.
    as_of = end_dt.strftime("%Y-%m-%d")
    pit = min(as_of, _fred_today())
    ...
    if not points:
        ...
        return header + (
            f"\nNo observations for {echoed_series} in this window at the {pit} "
